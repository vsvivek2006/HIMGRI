...
const Header: React.FC = () => {
  ...
  const [isSearchOpen, setIsSearchOpen] = useState(false); // ✅ Added Search State
    ...
    <div className="w-full font-sans sticky top-0 z-[100] shadow-sm bg-white">
      ...
      <div className="bg-[#C41E3A] text-white text-[10px] md:text-[11px] py-1.5 px-4 flex justify-between items-center tracking-wider">
        ...
        <div 
          ...
          className="hidden md:flex items-center gap-1.5 cursor-pointer hover:opacity-80 transition-opacity font-black uppercase"
        >
          <Mail size={12} />
      ...
      <header className="bg-white border-b border-gray-100 py-2 px-4 md:px-8 relative">
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-8">
              ...
              {categories.map((item) => (
                <li key={item.name}>
                  <NavLink
                    to={item.href}
                    className={({ isActive }) =>
                      `text-[10px] font-black uppercase tracking-[0.15em] transition-all hover:text-[#C41E3A] py-1 relative ${
          ...
          <div className="flex items-center gap-3 md:gap-5">
            ...
            <button 
              ...
              className="p-2 text-gray-500 hover:text-[#C41E3A] transition-colors"
            >
            ...
            <button 
              ...
              className="relative flex items-center gap-2 group p-2 hover:bg-gray-50 rounded-xl transition-colors"
            >
              <div className="relative">
                <ShoppingCart size={20} className="text-gray-900 group-hover:text-[#C41E3A] transition-colors" strokeWidth={2.5} />
                {cartCount > 0 && (
                  <span className="absolute -top-2 -right-2 bg-[#C41E3A] text-white text-[9px] font-black h-4 w-4 rounded-full flex items-center justify-center border border-white">
                    {cartCount}
                  </span>
                )}
              </div>
              <span className="hidden xl:block text-[10px] font-black text-gray-900 uppercase tracking-widest">My Jar</span>
            </button>
        ...
        {/* ✅ SLIM OVERLAY SEARCH BAR - Pops out when active */}
        {isSearchOpen && (
          <div className="absolute top-full left-0 w-full bg-white border-b border-gray-100 p-4 animate-in slide-in-from-top duration-300">
            <div className="max-w-3xl mx-auto relative">
              ...
              />
              <button className="absolute right-4 top-1/2 -translate-y-1/2 text-[#C41E3A]">
                <Search size={18} />
              </button>
            </div>
          </div>
      ...
      {isMenuOpen && (
        <div className="lg:hidden bg-white fixed inset-0 z-[110] p-8 animate-in slide-in-from-right overflow-y-auto">
          <div className="flex justify-between items-center mb-10">
            <div className="font-black text-xl tracking-tighter italic uppercase text-gray-900">Menu</div>
            <button onClick={() => setIsMenuOpen(false)} className="p-2 bg-red-50 rounded-full text-[#C41E3A]">
              <X size={20} />
            </button>
          </div>
          <ul className="space-y-5">
            {categories.map((item) => (
              <li key={item.name} onClick={() => setIsMenuOpen(false)}>
                <Link to={item.href} className="text-2xl font-black text-gray-900 uppercase italic tracking-tighter hover:text-[#C41E3A] transition-colors">
                  {item.name}
                </Link>
              </li>
            ))}
